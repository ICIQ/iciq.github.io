...
tags: numerical-analysis Runge-Kutta
published: false
---
Internal stability deals with the growth of errors (such as boundoff) introduced at the Runge-Kutta stages during a single Runge-Kutta step.  It is usually important only for methods with a large number of stages, since that is when the internal amplification factors can be large.  Here we demonstrate some tools for studying internal stability in NodePy.
...
Aside: *In the solution of PDEs, large spatial truncation errors enter at each stage.  Does this mean internal stability becomes more significant?  How does this relate to stiff accuracy analysis and order reduction?*

## Internal stability functions

The internal stability functions are (*add a derivation here*):

<center>$\theta(z) = z b^T (I-zA)^{-1}.$</center>

Notice that $\theta(z)$ is a vector of functions.  Let's compute them for the classical RK4 method:
...
Let's think about when things can go wrong in terms of internal instability.  If $|\theta(z)|$ is of the order $1/\epsilon_{machine}$, then roundoff errors could be amplified so much that they destroy the accuracy of the computation.  More specifically, we should be concerned if $|\theta(z)|$ is of the order $tol/\epsilon_{machine}$ where $tol$ is our desired error tolerance.  Of course, we only care about values of $z$ that lie inside the absolute stability region $S$, since internal stability won't matter if the computation is not absolutely stable.
...
Remark: *Do SSP RK methods always (necessarily) have small amplification factors?  Can we prove it?*
...
<span class="n">plot</span><span class="p">(</span><span class="n">ampfac</span><span class="p">)</span>
</pre></div>
